...
- `packages/@somedaycode/core`: Core React components
- `packages/@somedaycode/utils`: Shared React utilities
- `packages/@somedaycode/tsconfig`: Shared `tsconfig.json`s used throughout the Turborepo
- `packages/eslint-preset-somedaycode`: ESLint preset
...
For `somedaycode-core`, the `build` command is the following:

```bash
tsup src/index.ts --format esm,cjs --dts --external react
```

`tsup` compiles `src/index.ts`, which exports all of the components in the design system, into both ES Modules and CommonJS formats as well as their TypeScript types. The `package.json` for `somedaycode-core` then instructs the consumer to select the correct format:

```json:somedaycode-core/package.json
{
...
Run `yarn build` to confirm compilation is working correctly. You should see a folder `somedaycode-core/dist` which contains the compiled output.
...
Each file inside of `somedaycode-core/src` is a component inside our design system. For example:

```tsx:somedaycode-core/src/Button.tsx
...
```tsx:somedaycode-core/src/index.ts
import * as React from "react";
...
- Support using module path aliases like `@somedaycode-core` for imports
- Write MDX for component documentation pages

For example, here's the included Story for our `Button` component:

```js:apps/docs/stories/button.stories.mdx
import { Button } from '@somedaycode-core/src';
...
Turborepo runs the `build` script for all publishable packages (excluding docs) and publishes the packages to npm. By default, this example includes `somedaycode` as the npm organization. To change this, do the following:

- Rename folders in `packages/*` to replace `somedaycode` with your desired scope
- Search and replace `somedaycode` with your desired scope
- Re-run `yarn install`
